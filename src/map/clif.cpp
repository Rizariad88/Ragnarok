...
static uint8 clif_add_random_options(struct ItemOptions buf[MAX_ITEM_RDM_OPT], struct item* it, struct item_data* id) {
#else
static uint8 clif_add_random_options(struct ItemOptions buf[MAX_ITEM_RDM_OPT], struct item* it) {
	...
	for( int i = 0; i < MAX_ITEM_RDM_OPT; i++ ){
		...
		if (battle_config.storm_item_durability && battle_config.storm_item_durability_random_option && !it->option[i].id && (i == 0 || it->option[i - 1].id) && (id->type == IT_WEAPON || (id->equip & battle_config.storm_item_armor_mask))) {
			buf[i].index = STORM_ITEM_RANDOPT_ID;
			buf[i].value = it->durability / 10000;
			buf[i].param = 0;
			count++;
		} else {
#endif
...
#ifdef STORM_ITEM_DURABILITY
		clif_add_random_options(p.option_data, &sd->inventory.u.items_inventory[n], sd->inventory_data[n]);
#else
...
#if PACKETVER >= 20150226
#ifdef STORM_ITEM_DURABILITY
	p->option_count = clif_add_random_options(p->option_data, it, id);
#else
...
#if PACKETVER >= 20150226
#ifdef STORM_ITEM_DURABILITY
		clif_add_random_options(p.option_data, &sd->inventory.u.items_inventory[index], sd->inventory_data[index]);
#else
...
#if PACKETVER >= 20150226
#ifdef STORM_ITEM_DURABILITY
	clif_add_random_options(p.option_data, i, sd->inventory_data[index]);
#else
...
#if PACKETVER >= 20150226
#ifdef STORM_ITEM_DURABILITY
	clif_add_random_options(p.option_data, &sd->cart.u.items_cart[n], itemdb_search(sd->cart.u.items_cart[n].nameid));
#else
...
#ifdef STORM_ITEM_DURABILITY
		clif_add_random_options(p->items[i].option_data, &vsd->cart.u.items_cart[index], itemdb_search(vsd->cart.u.items_cart[index].nameid));
#else
...
#if PACKETVER >= 20150226
#ifdef STORM_ITEM_DURABILITY
		clif_add_random_options(p->items[i].option_data, &sd->cart.u.items_cart[index], itemdb_search(sd->cart.u.items_cart[index].nameid));
#else
...
static bool clif_process_message(map_session_data* sd, bool whisperFormat, char* out_name, char* out_message, char* out_full_message ){
	...
	if (sd->sc.cant.chat || (sd->state.block_action & PCBLOCK_CHAT) || sd->special_state.no_chat)
...
void clif_parse_NpcBuyListSend( int fd, map_session_data* sd ){
	...
	if (sd->state.trading)
		...
		result = storm_openshop_buy(sd, items);
...
#ifdef STORM_ITEM_DURABILITY
		clif_add_random_options(p.optionData, item, sd->inventory_data[server_index(index)]);
#else
...
void clif_Mail_read( map_session_data *sd, int mail_id ){
	...
	if( i == MAIL_MAX_INBOX ) {
		...
		for( int j = 0; j < MAIL_MAX_ITEM; j++ ){
			...
			std::shared_ptr<item_data> data = item_db.find(item->nameid);
			struct item_data* data2 = itemdb_search(item->nameid);
...
#ifdef STORM_ITEM_DURABILITY
				clif_add_random_options(mailitem->option_data, item, data2);
#else
...
void clif_parse_cashshop_buy( int fd, map_session_data *sd ){
	...
	bool openshop = sd->openshop != NULL;
	...
	if (openshop) {
		storm_openshop_cashbuy(sd, p->kafraPoints, p->count, p->items);
	}else
...
#if PACKETVER >= 20150226
#ifdef STORM_ITEM_DURABILITY
		clif_add_random_options(p->items[i].option_data, &it, itemdb_search(it.nameid));
#else
 ...
 **/
/*==========================================
 * Spellbook list [Updated for modern rAthena standards]
 *------------------------------------------*/
void clif_spellbook_list(map_session_data *sd) {
    nullpo_retv(sd);
    ...
    if (count > 0) {
        ...
        // End the spellbook reading status effect and notify the player of the failure
...
void clif_inventory_select_list(map_session_data* sd, int* positions, int count)
{
	int i, c;
	int fd;
...
/// 0442 <Length>.W <count>.L <Skill_list>.W (ZC_SKILL_SELECT_REQUEST).
int clif_skill_select_list(map_session_data *sd, unsigned short* skills, int count) {
	int i;
	int fd;
...
void clif_openshop_buylist(map_session_data *sd)
{
	int fd, i, c;
	bool discount;
	...
	{
		struct item_data* id = itemdb_search(sd->openshop->shop_item[i].nameid);
...
/// 0287 <packet len>.W <cash point>.L { <sell price>.L <discount price>.L <item type>.B <name id>.W }*
/// 0287 <packet len>.W <cash point>.L <kafra point>.L { <sell price>.L <discount price>.L <item type>.B <name id>.W }* (PACKETVER >= 20070711)
void clif_openshop_cashlist(map_session_data *sd)
{
	int fd, i, cost[2] = { 0, 0 };
	openshoplist* shop;
#if PACKETVER < 20070711
 ...
 * @param sd the player
 */
void clif_npc_getareachar_unit(map_session_data* sd)
{
	nullpo_retv(sd);
 ...
 * @param nd the shop
 */
void clif_bazaarshop_open(map_session_data* sd, struct npc_data* nd)
{
	int fd, i, j, c;
	struct item_data* id;
	bazaarlist list;
	...
	for (i = 0, c = 0; i < list.count; i++)
	{
		...
		if (list.items[i].nameid == 0 || !(id = itemdb_search(list.items[i].nameid)))
			continue;
...
#ifdef STORM_ITEM_DURABILITY
	clif_add_random_options(p.option_data, item, sd->inventory_data[index]);
#else
