...
static void pc_bonus_expansion(struct map_session_data* sd, int type, int val)
{
	...
	case SP_NOEQUIP:
		if (sd->state.lr_flag != 2) {
			...
			sd->indexed_bonus.noequip[val] = 1;
			if (index > -1)
...
static void pc_bonus2_expansion(struct map_session_data* sd, int type, int value1, int value2)
{
	...
	case SP_BREAK_EQUIP:
		...
		if (sd->state.lr_flag != 2)
			sd->indexed_bonus.breakequip[value1] += value2;
		break;
	case SP_MULTIDROP:
		if (sd->state.lr_flag != 2)
	...
	case SP_WEAPONDEF:
		if (sd->state.lr_flag != 2) {
			...
			sd->indexed_bonus.weapondef[value1] += value2;
		}
		break;
	case SP_SKILLITEMCOST:
		if (sd->state.lr_flag != 2)
	...
	case SP_DURABILITYDEF:
		if (sd->state.lr_flag != 2) {
			...
			sd->indexed_bonus.durabilitydef[value1] += value2;
		}
		break;
	case SP_SKILLREQUIRECOST:
		if (sd->state.lr_flag != 2)
...
uint8 pc_isequip(map_session_data *sd,int n)
{
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (item->equip & mask && sd->indexed_bonus.noequip[i])
			return ITEM_EQUIP_ACK_FAIL;
	}
...
void pc_cell_basilica(map_session_data *sd) {
	...
	if (!map_getcell(sd->bl.m,sd->bl.x,sd->bl.y,CELL_CHKBASILICA)) {
		if (sd->sc.data[SC_BASILICA])
			status_change_end(&sd->bl, SC_BASILICA);
	}
