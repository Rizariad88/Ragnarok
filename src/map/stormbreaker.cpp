 ...
 */
static bool storm_bazaar_complete(map_session_data* sd, bazaar* entry)
{
	int i, j;
	bazaarunlocked* buy;
	struct item_data* id;
	...
	if (!entry->nameid || !(id = itemdb_search(entry->nameid)))
		return false;
	...
	if (battle_config.storm_bazaar_notice == 2) {
		...
		snprintf(message, sizeof(message), msg_txt(sd, 1524), id->ename.c_str(), entry->max);
...
void storm_bazaar_sold(map_session_data* sd, uint16 item, uint16 amount)
{
    ...
    if (item < 1 || !item_db.find(item) || !amount)
        return;
    ...
    for (check = search; check != nullptr; check = check->next)
    {
        ...
        if (i == MAX_BAZAAR_ITEMS || entry->flags & BZF_REPEAT)
            continue;  // ?? Fixed: Don't break the loop
    }
    ...
    if (sold->nameid) {
        if (USHRT_MAX - sold->amount < amount)  // ?? Fixed: Prevent overflow
            sold->amount = USHRT_MAX;
        else
            sold->amount += amount;
    } else {
        sold->nameid = item;
    ...
    bazaarsearch* original_search = search;  // ?? Fixed: Store original pointer
    ...
    search = original_search;  // ?? Fixed: Restore pointer for safety

    if (!completed)
        return;

    if (battle_config.storm_bazaar_notice == 1) {
        char message[CHAT_SIZE_MAX] = {0};  // ?? Fixed: Initialized properly
    ...
    if (battle_config.storm_bazaar_cutin && sd->bazaar_timer_id == INVALID_TIMER) {
        ...
        sd->bazaar_timer_id = add_timer(gettick() + battle_config.storm_bazaar_cutin, 
                                        storm_bazaar_clearcutin, sd->bl.id, 0); // ?? Fixed: Replaced NULL with 0
    }
}
...
bazaarlist storm_bazaar_list(map_session_data* sd, struct npc_data* nd)
{
	...
	for (i = 0; list.count < MAX_BAZAAR_SHOP && i < MAX_BAZAAR_ITEMS; i++)
	{
		...
		if (entry == nullptr || entry->nameid == 0 || !(id = itemdb_search(entry->nameid)))
			continue;
...
void storm_bazaar_purchase(map_session_data* sd, const uint8* data, uint16 count)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		for (j = 0; j < MAX_BAZAAR_ITEMS; j++)
		{
			...
			if (!(id = itemdb_search(entry->nameid)))
				continue;
...
static bool storm_readdb_bazaar_sub(char** row, int columns, int line)
{
	...
	if (nameid < 0 || !item_db.find(nameid)) {
	...
	for (i = 0; i < MAX_BAZAAR_ENTRY && i < (columns - 5); i++)
	{
		...
		if (p == nullptr || amount < 1 || item < 1 || !item_db.find(item)) {
...
void storm_openshop(map_session_data* sd, struct npc_item_list* items, uint16 count, openshop_flag flags, uint16 item, const char* points)
{
	...
	if (flags&OSHP_ITEMSHOP && (!item || !item_db.find(item))) {
	...
	for (i = 0; i < count; i++) {
		...
		if (!it->nameid || !it->value || !item_db.find(it->nameid)) {
...
e_purchase_result storm_openshop_buy(map_session_data* sd, std::vector<s_npc_buy_list> list)
{
	...
	if (list.empty() || !sd->openshop || !sd->openshop->shop_item)
		return e_purchase_result::PURCHASE_FAIL_COUNT;
	...
	for (i = 0, add = 0; i < list.size(); i++)
	{
		...
		if (npc_shop_discount2(shop->type, shop->discount))
 ...
 */
static int storm_openshop_cashbuy_sub(map_session_data* sd, openshoplist* shop, int price, int points)
{
	int cost[2] = { 0, 0 };
	...
	switch (shop->type)
	{
		...
		case NPCTYPE_ITEMSHOP: {
			int i, amount = price;
			struct item_data* id = itemdb_search(shop->itemshop_nameid);
			...
			if (cost[1] < points || cost[0] < (price - points))
			{
				...
				snprintf(message, sizeof(message), msg_txt(sd, 712), id->ename.c_str(), id->nameid);
...
int storm_openshop_cashbuy(map_session_data* sd, int points, int count, struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST_sub* item_list){
	...
	for (i = 0; i < count; i++)
	{
		int id = item_list[i].itemId;
		int amount = item_list[i].amount;

		if (amount < 1 || !item_db.find(id))
		...
		id = item_list[i].itemId = shop->shop_item[j].nameid;
...
void storm_openshop_currency(map_session_data* sd, int* cost, bool display)
{
	...
	switch (sd->openshop->type)
	{
		...
		case NPCTYPE_ITEMSHOP: {
			int total = 0, i;
			struct item_data* id = itemdb_search(sd->openshop->itemshop_nameid);
			...
			if (id)
			{
				if (display)
				{
					...
					snprintf(message, sizeof(message), msg_txt(sd, 714), id->ename.c_str(), id->nameid);
...
void storm_itemdamage(map_session_data* sd, struct block_list* bl, uint16 type, uint64 damage, uint16 hits)
{
	...
	for (i = 0, count = 0; i < EQI_MAX; i++) {
		...
		reduce = sd->bonus.durabilitydef + sd->indexed_bonus.durabilitydef[i];
 ...
 */
static void storm_itempassive_sub(map_session_data* sd, uint16 nameid, item_change_flag flag)
{
	int i;
	struct status_change* sc;
	struct item_data* id;
		...
		for (i = 0; i < ise->count; i++)
		{
			...
			case ICF_ITEM_REMOVE:
				if (sce && sce->passive && !(st->flag & IPF_EQUIP) && pc_search_inventory(sd, nameid) < 0)
					status_change_end(&sd->bl, (sc_type)st->type);
				break;
...
void storm_itempassive_renew(map_session_data* sd)
{
	...
	for (i = 0; i < SC_MAX; i++)
	{
		...
		status_change_end(&sd->bl, (sc_type)i);
...
static bool storm_readdb_itemscript_sub(char** row, int32 columns, int32 line)
{
	...
	if (item_id <= 0 || !(id = itemdb_search(item_id))) {
...
static bool storm_readdb_itemstatus_sub(char** row, int32 columns, int32 line)
{
	struct item_data* id;
	int64 status, j;
	int i = 0, val1 = 0, val2 = 0, val3 = 0, val4 = 0;
	unsigned short item_id;
	item_passive_flag flag = IPF_NONE;
	item_status_entry* ise;
