...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			struct item_data* id = itemdb_search(md->db->dropitem[i].nameid);
...
#ifdef STORM_ITEM_DURABILITY
			ditem = mob_setdropitem(&md->db->dropitem[i], count, md->mob_id, id);
#else
...
#ifdef STORM_ITEM_DURABILITY
					mob_item_drop(md, dlist, mob_setdropitem(&mobdrop, 1, md->mob_id, itemdb_search(dropid)), 0, drop_rate, homkillonly || merckillonly);
#else
		...
		if( mapdrops != nullptr ){
			...
			for( const auto& it : mapdrops->globals ){
				if( rnd_chance( it.second->rate, 10000 ) ){
					struct item_data* id = itemdb_search(it.second->nameid);
					mob_item_drop( md, dlist, mob_setdropitem( it.second.get(), 1, md->mob_id, id), 0, it.second->rate, homkillonly || merckillonly );
				}
			}
