...
int status_change_clear(struct block_list* bl, int type)
{
	...
	for (const auto &it : status_db) {
		...
		const std::unordered_set<int> ignored_types = {
			SC_CP_WEAPON, SC_CP_SHIELD, SC_CP_ARMOR, SC_CP_HELM, SC_DEEPSLEEP,
			SC_BURNING, SC_FREEZING, SC_CRYSTALIZE, SC_TOXIN, SC_PARALYSE,
			SC_VENOMBLEED, SC_MAGICMUSHROOM, SC_DEATHHURT, SC_PYREXIA,
			SC_OBLIVIONCURSE, SC_LEECHESEND, SC_MARSHOFABYSS, SC_MANDRAGORA,
			SC_HALLUCINATION, SC_QUAGMIRE, SC_SIGNUMCRUCIS, SC_DECREASEAGI,
			SC_SLOWDOWN, SC_MINDBREAKER, SC_WINKCHARM, SC_STOP, SC_ORCISH,
			SC_STRIPWEAPON, SC_STRIPSHIELD, SC_STRIPARMOR, SC_STRIPHELM,
			SC_BITE, SC_ADORAMUS, SC_VACUUM_EXTREME, SC_FEAR, SC_MAGNETICFIELD,
			SC_NETHERWORLD
		};

		if (sd && sd->special_state.keep_buffs && ignored_types.count(type))
			continue;
 ...
 */
#ifdef STORM_ITEM_STATUS
int status_change_end(struct block_list* bl, enum sc_type type, int tid, bool passive)
#else
...
#endif
{
	...
	// Status bonus
	if (!calc_flag.test(SCB_BASE) && sd && !sd->statusbonus.empty())
	{
		for (const auto& it : sd->statusbonus)
		{
			if (it.type == type) {
