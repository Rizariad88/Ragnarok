...
#ifdef STORM_ITEM_DURABILITY
	SqlStmt_BindColumn(stmt, 12, SQLDT_UINT, &entry.item.durability, 0, NULL, NULL);
	pos++;
#endif
	for( j = 0; j < MAX_SLOTS; ++j )
		SqlStmt_BindColumn(stmt, pos +j, SQLDT_UINT, &entry.item.card[j], 0, NULL, NULL);
	for( j = 0; j < MAX_ITEM_RDM_OPT; ++j ) {
		SqlStmt_BindColumn(stmt, pos +MAX_SLOTS+j*3, SQLDT_SHORT, &entry.item.option[j].id, 0, NULL, NULL);
		SqlStmt_BindColumn(stmt, pos +1+MAX_SLOTS+j*3+1, SQLDT_SHORT, &entry.item.option[j].value, 0, NULL, NULL);
		SqlStmt_BindColumn(stmt, pos +2+MAX_SLOTS+j*3+2, SQLDT_CHAR, &entry.item.option[j].param, 0, NULL, NULL);
	}
